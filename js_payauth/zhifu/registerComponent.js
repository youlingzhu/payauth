function Component_list() {
    ...
    this.id_doc_type = function (data) {
        ...
        if (subject_boolean == false) {
            ...
            ul.children[0].style.width = '148px';
    ...
    this.id_card_copy = function (data) {
        ...
        if (subject_boolean) {
            ...
            document.getElementById('shenfenzhengzhaopianzhengmian').style.paddingLeft = '0px';
    ...
    this.merchant_shortname = function (data, componentNextPro) {
        ...
        img.src = '../img_payauth/zhifu_jingyingxinxi.png';
    ...
    this.service_phone = function (data, componentNextPro) {
        ...
        this.input_box_id_card(data, dom);
        is.setAttribute('class', 'ico-msg-s');
        img.src = '../img_payauth/trans_record.png';
        div.appendChild(divImg);
        divImg.appendChild(img);
        // divImg.appendChild(imgs);
        divImg.appendChild(p);
    ...
    this.sales_scenes_type = function (data) {
        ...
        if (subject_boolean == true) {
            ...
            document.getElementById('matation_form-item_shenfenzhengfour_uls').children[0].style.width = '148px';
        }
    ...
    this.input_box_id_card = function (data, dom, componentNextPro) { // componentNextPro 是一个函数；
        ...
        input.onblur = function () {
            ...
            if (data.caption == '身份证姓名' && (subject_type == 'SUBJECT_TYPE_ENTERPRISE' || subject_type == 'SUBJECT_TYPE_INDIVIDUAL')) {
                var getihu_farendaibiao = document.getElementById('getihu_farendaibiao');
                var values = getihu_farendaibiao.value;
                if (data.value != values) {
                    p.style.display = 'block';
                    p.innerHTML = '请填写正确的身份证姓名（即与营业执照上的个体户经营者/法人姓名相同的身份证件）';
                    input.setAttribute("isNext", "nextStep");
                    return;
                } else {
                    p.style.display = 'none';
            ...
            } else if (data.caption == '身份证姓名' && (subject_type == 'SUBJECT_TYPE_INSTITUTIONS' || subject_type == 'SUBJECT_TYPE_OTHERS')) {
                var getihu_farendaibiao = document.getElementById('dengji_faren_xingming');
                var values = getihu_farendaibiao.value;
                if (data.value != values) {
                    p.style.display = 'block';
                    p.innerHTML = '请填写正确的身份证姓名（即与登记执照上的个体户经营者/法人姓名相同的身份证件）';
                    input.setAttribute("isNext", "nextStep");
                    return;
                } else {
                    p.style.display = 'none';
